...
# changed struct silently leaves stale objects behind and layouts can diverge.
...
# widget toolkit. A couple are embedded in the kernel image so there is always
# something to run even without a disk.
USER_DIR    := $(BUILD)/user
...
# The toolchain probe below is declared before 'all', so state the default goal.
.DEFAULT_GOAL := all

# Fail with an explanation rather than a wall of assembler errors when the
