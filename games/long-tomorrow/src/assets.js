...
const Assets={
  ...
  gltf(p){
    if(!this.gltfs.has(p))this.gltfs.set(p,this.loader().loadAsync(this.url(p)).then(g=>{this.loaded.set(p,g);return g;}).catch(e=>{console.warn('asset failed',p,e);this.failed.add(p);return null;}));
  ...
  hdr(p){
    if(!this.hdrs.has(p))this.hdrs.set(p,fetch(this.url(p)).then(r=>{if(!r.ok)throw new Error(r.status);return r.arrayBuffer();}).then(buf=>{
