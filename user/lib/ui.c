...
ui_window *ui_window_open(const char *title, int width, int height,
                          unsigned flags) {
  ui_window *window;
  ...
  id = syscall3(SYS_GUI_CREATE, (int)((flags << 16) | (unsigned)width), height,
                (int)title);
  if (id < 0) {
    free(window);
    return NULL;
...
void ui_redraw(ui_window *window) { window->dirty = 1; }
...
void ui_frame_wait(void) { sleep_ms(16); }
...
void ui_clip_set(ui_window *window, int x, int y, int w, int h) {
  ...
  if (x + w > window->surface.width)
    w = window->surface.width - x;
  if (y + h > window->surface.height)
    h = window->surface.height - y;
...
void ui_pixel(ui_window *window, int x, int y, uint32_t color) {
  ...
  if (x < clip->x || y < clip->y || x >= clip->x + clip->w ||
      y >= clip->y + clip->h)
    return;
...
void ui_fill(ui_window *window, int x, int y, int w, int h, uint32_t color) {
  ...
  for (int row = 0; row < h; row++) {
    uint32_t *line =
        window->surface.pixels + (y + row) * window->surface.stride + x;
  ...
  for (int row = 0; row < h; row++) {
    ...
    for (int col = 0; col < w; col++) {
      ...
      uint32_t g = ((((color >> 8) & 0xFF) * alpha) +
                   ...
                   255;
      uint32_t b =
          (((color & 0xFF) * alpha) + ((dst & 0xFF) * (255 - alpha))) / 255;
...
void ui_round_fill(ui_window *window, int x, int y, int w, int h, int radius,
                   uint32_t color) {
  if (radius * 2 > w)
    radius = w / 2;
  if (radius * 2 > h)
  ...
  for (int dy = 0; dy < radius; dy++) {
    ...
    while (dx < radius && (radius - dy) * (radius - dy) +
                                  (radius - dx - 1) * (radius - dx - 1) >
                              radius * radius)
      dx++;
...
void ui_gradient(ui_window *window, int x, int y, int w, int h, uint32_t top,
                 uint32_t bottom) {
  if (h <= 0)
    return;
  ...
  for (int row = 0; row < h; row++) {
    ...
    uint32_t r = ((((top >> 16) & 0xFF) * (h - 1 - row)) +
                  (((bottom >> 16) & 0xFF) * row)) /
                 divisor;
    uint32_t g = ((((top >> 8) & 0xFF) * (h - 1 - row)) +
                  (((bottom >> 8) & 0xFF) * row)) /
                 divisor;
    uint32_t b =
        (((top & 0xFF) * (h - 1 - row)) + ((bottom & 0xFF) * row)) / divisor;

    ui_fill(window, x, y + row, w, 1, 0xFF000000u | (r << 16) | (g << 8) | b);
  }
}

void ui_line(ui_window *window, int x0, int y0, int x1, int y1,
             uint32_t color) {
...
void ui_text_scaled(ui_window *window, int x, int y, const char *text,
    ...
    for (int row = 0; row < UI_GLYPH_H; row++) {
      ...
      for (int col = 0; col < UI_GLYPH_W; col++) {
        ...
        if (scale == 1)
          ...
          ui_fill(window, x + col * scale, y + row * scale, scale, scale,
                  color);
      }
    }
    x += UI_GLYPH_W * scale;
...
void ui_text(ui_window *window, int x, int y, const char *text,
             uint32_t color) {
  ui_text_scaled(window, x, y, text, color, 1);
}

void ui_text_center(ui_window *window, int x, int y, int w, const char *text,
                    uint32_t color) {
...
int ui_text_width(const char *text) { return (int)strlen(text) * UI_GLYPH_W; }
...
int ui_button_colored(ui_window *window, int x, int y, int w, int h,
  ...
  if (window->active == id && hover)
    face =
        UI_RGBA((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, 255) &
        0xFF9F9F9Fu;
  else if (hover)
    face = color + 0x00101010u;

  ui_round_fill(window, x, y, w, h, 8, face);
  ui_frame(window, x, y, w, h, UI_BORDER);
  ui_text_center(window, x, y + (h - UI_GLYPH_H) / 2, w, label, UI_TEXT);
  return clicked;
}

int ui_button(ui_window *window, int x, int y, int w, int h,
              const char *label) {
...
int ui_textbox(ui_window *window, int x, int y, int w, const char *label,
  ...
  ui_round_fill(window, x, y, w, height, 7, UI_SURFACE);
  ui_frame(window, x, y, w, height,
           window->focus == id ? UI_ACCENT : UI_BORDER);
  ui_clip_set(window, x + 4, y, w - 8, height);
  {
    ...
    if (window->focus == id)
      ui_fill(window, x + 6 - offset + text_width, y + 4, 2, UI_GLYPH_H,
              UI_ACCENT_LIGHT);
  }
  ui_clip_reset(window);
  return submitted;
...
int ui_list(ui_window *window, int x, int y, int w, int h,
  ...
  if (count > visible) {
    ...
    ui_round_fill(window, x + w - 8, y + 2 + position, 6, thumb, 3,
                  UI_PANEL_LIGHT);
  }
...
void ui_panel(ui_window *window, int x, int y, int w, int h,
              const char *title) {
  ui_round_fill(window, x, y, w, h, 10, UI_PANEL);
  ui_frame(window, x, y, w, h, UI_BORDER);
  if (title) {
...
void ui_progress(ui_window *window, int x, int y, int w, int h, int percent,
                 uint32_t color) {
  if (percent < 0)
    percent = 0;
  if (percent > 100)
