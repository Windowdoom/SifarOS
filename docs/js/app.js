...
// Base path of the app: "/" on the Node server, "/SifarOS/" on GitHub Pages.
...
async function loadRules() {
  // Server mode first (live rules + model availability); fall back to the static
  // data bundle used on GitHub Pages or when offline.
  try {
  ...
  try {
    const res = await fetch(`${BASE}data/rules-${encodeURIComponent(state.jurisdiction)}.json`);
    const data = await res.json();
    rules = data.rules;
    verification = data.verification || verification;
    modelAvailable = false;
  } catch (e) {
    // No network and no cache yet — views guard for null rules.
  }
}
async function fetchJurisdictions() {
  ...
  const res = await fetch(`${BASE}data/jurisdictions.json`);
  return res.json();
}
