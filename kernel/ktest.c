...
static void test_div64(void) {
  ...
  check(big % 999983ull == (big - (big / 999983ull) * 999983ull),
        "64 bit modulo");
  check((uint64_t)(-1ll) / 2ull == 9223372036854775807ull,
        "unsigned wide division");
}
...
static void test_paging(void) {
  ...
  check(vmm_map(scratch, frame, PTE_PRESENT | PTE_WRITE) == 0,
        "RW+NX mapping succeeds");
...
static void test_fs(void) {
  struct fs_node *node;
  char buffer[128], path[FS_PATH_MAX];
  ssize_t n;
  suite("fs");
  ...
  buffer[n > 0 ? n : 0] = '\0';
  check(n == 11 && strcmp(buffer, "hello world") == 0,
  ...
  check(vfs_truncate(node, 5) == 0 && node->size == 5,
  ...
  check(vfs_lookup(NULL, "/tmp/ktest/file.txt") == node,
        "absolute lookup works");
  ...
  check(vfs_lookup(NULL, "/tmp/ktest/missing") == NULL,
  ...
  check(vfs_path_of(node, path, sizeof(path)) == 0 &&
            strcmp(path, "/tmp/ktest/file.txt") == 0,
        "path_of rebuilds the path");
  check(vfs_write_file("/tmp/ktest/helper.txt", "abc", 3) == 0,
        "write_file helper");
  check(vfs_append_file("/tmp/ktest/helper.txt", "def", 3) == 0,
        "append_file helper");
  n = vfs_read_file("/tmp/ktest/helper.txt", buffer, sizeof(buffer));
  buffer[n > 0 ? n : 0] = '\0';
  check(strcmp(buffer, "abcdef") == 0, "append lands after original bytes");
  check(vfs_unlink(NULL, "/tmp/ktest") == 0, "removing a directory works");
  check(vfs_lookup(NULL, "/tmp/ktest/file.txt") == NULL,
        "children disappear with directory");
...
static void test_sched(void) {
  ...
  thread_sleep_ms(120);
  after = timer_ms();
  check(after - before >= 110, "sleep blocks for at least requested time");
  check(thread_current() != NULL && thread_current()->tid >= 0,
        "current thread is identifiable");
...
static void test_interrupts(void) {
  ...
  while (timer_ticks() == start && spins < 100000000)
    spins++;
  check(timer_ticks() > start, "timer interrupt is firing");
  check(timer_hz() == 100, "PIT runs at configured rate");
}
...
static void test_graphics(void) {
  ...
  check(pixels[3 * 32 + 4] == RGB(0, 0, 0), "fill_rect leaves outside alone");
  gfx_clip_set(&surface, 16, 0, 16, 16);
  gfx_fill_rect(&surface, 0, 0, 32, 16, RGB(0, 255, 0));
  check(pixels[0] == RGB(0, 0, 0), "clipping protects left half");
  check(pixels[20] == RGB(0, 255, 0), "clipping paints right half");
  ...
  check(gfx_font()[('A' * GLYPH_H) + 2] != 0, "ROM font has glyph data");
}

static void test_elf(void) {
  const struct embedded_program *program = program_find("hello");
  suite("elf");
  check(program != NULL, "embedded program is present");
  if (!program)
    return;
  check(elf_is_valid(program->start, (size_t)(program->end - program->start)),
        "embedded image is valid ELF");
  check(!elf_is_valid((const uint8_t *)"not an elf at all", 17),
        "rubbish is rejected");
  {
    const struct elf32_ehdr *h = (const struct elf32_ehdr *)program->start;
    check(h->entry >= USER_MIN && h->entry < USER_MAX,
          "entry point is in user range");
    check(h->phnum > 0, "image has program headers");
  }
}
static void test_processes(void) {
  const struct embedded_program *program = program_find("hello");
  int pid, code = -1, before = proc_count();
  suite("processes");
  check(proc_current() != NULL, "current process exists");
  check(proc_kernel()->pid == 0, "kernel is process zero");
  if (!program)
    return;
  pid = proc_spawn_image("ktest-hello", program->start,
                         (size_t)(program->end - program->start), 1,
                         (const char *const[]){"ktest-hello"});
  check(pid > 0, "program can be loaded and started");
  if (pid <= 0)
    return;
  check(proc_by_pid(pid) != NULL, "new process is in table");
  check(proc_count() > before, "process count went up");
  check(proc_wait(pid, &code) == 0, "waiting succeeds");
  check(code == 7, "exit status comes back");
  check(proc_by_pid(pid) == NULL, "process is gone once reaped");
}
...
static void test_address_spaces(void) {
  ...
  check(space.pd[3][heap_pd] == kernel->pd[3][heap_pd],
        "kernel heap page table is shared");
  ...
  check(vmm_translate_in(&space, user_page) == frame,
        "other space translates it");
  check(vmm_translate_in(kernel, user_page) == 0,
        "kernel space does not see private user mapping");
...
static void test_disk(void) {
  ...
  suite("disk");
  if (!disk) {
    check(0, "ATA disk is attached");
    return;
  }
  check(disk->sectors > 0, "disk reports a size");
  ...
  check(first[510] == 0x55 && first[511] == 0xAA, "boot signature is on disk");
  {
    uint32_t scratch_lba = disk->sectors - 1;
    memset(second, 0, SECTOR_SIZE);
    check(disk->read(disk, scratch_lba, 1, second) == 0, "spare sector reads");
    ...
    check(disk->read(disk, scratch_lba, 1, second) == 0, "and reads again");
    {
      ...
      for (int i = 0; i < SECTOR_SIZE; i++)
        if (second[i] != (uint8_t)(i ^ 0x5A))
          intact = 0;
      check(intact, "round trip preserves data");
    }
  }
  kfree(first);
  kfree(second);
...
static void test_persistence(void) {
  ...
  suite("persistence");
  if (!sfs_mounted()) {
    check(0, "disk filesystem is mounted");
    return;
  }
  check(vfs_write_file("/tmp/ktest-disk.txt", payload, sizeof(payload) - 1) ==
            0,
        "file can be written");
  ...
  check(vfs_unlink(NULL, "/tmp/ktest-disk.txt") == 0, "file can be removed");
}
static void test_floating_point(void) {
  volatile double a = 3.5, b = 1.25;
  suite("floating point");
  check(fpu_present(), "FPU is available");
  check((int)(a * b * 100.0) == 437, "arithmetic works in kernel");
  a = 12345.75;
  sched_yield();
  check((int)(a * 2.0) == 24691, "registers survive context switch");
}
static void test_window_system(void) {
  ...
  check(wm_running() == (gfx_available() ? 1 : 0),
        "window server matches display state");
  count = wm_list_windows(list, GUI_MAX_WINDOWS);
  check(count >= 0, "window list can be read");
  if (count > 0) {
...
static void test_security(void) {
  ...
  security_event_record(SECURITY_EVENT_SYSCALL_VIOLATION, 42, 0xDEAD,
  ...
  check(security_event_count() == SECURITY_EVENT_LOG_CAPACITY,
        "event log is bounded");
  check(security_event_get(before, &event) == 0,
        "ring buffer remains readable after wrap");
  check(event.sequence > 1, "wrapped events keep monotonic sequence numbers");
  check(security_event_get(SECURITY_EVENT_LOG_CAPACITY, &event) < 0,
        "out of range event access is rejected");
}
...
int ktest_run(void) {
  ...
  kprintf("\nselftest: %d checks, %d passed, %d failed, %u ms\n\n", checks_run,
          checks_run - checks_failed, checks_failed,
