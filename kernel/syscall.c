...
static void denied(uint32_t syscall_number) {
  ...
  security_event_record(SECURITY_EVENT_SYSCALL_VIOLATION,
                        proc ? (uint32_t)proc->pid : 0, syscall_number,
...
static int32_t sys_stat(uint32_t path_ptr, uint32_t out_ptr) {
  ...
  if (!out || proc_copy_user_string((const char *)(uintptr_t)path_ptr, path,
                                    sizeof(path)) < 0)
    return -1;
  node = vfs_lookup(proc_current()->cwd, path);
  if (!node)
...
static int32_t sys_gui_create(uint32_t width, uint32_t height,
                              uint32_t title_ptr) {
  char title[GUI_MAX_TITLE];
  uint32_t flags = width >> 16;
  width &= 0xFFFF;
  if (proc_copy_user_string((const char *)(uintptr_t)title_ptr, title,
                            sizeof(title)) < 0)
    strlcpy(title, "Window", sizeof(title));
  return wm_create_window(proc_current(), (int)width, (int)height, title,
                          flags);
}
static int32_t sys_gui_info(uint32_t id, uint32_t p) {
  struct gui_window_info *o = USER_STRUCT(p, struct gui_window_info);
  return o ? wm_window_info(proc_current(), id, o) : -1;
}
static int32_t sys_gui_poll(uint32_t id, uint32_t p) {
  struct gui_event *o = USER_STRUCT(p, struct gui_event);
  return o ? wm_poll_event(proc_current(), id, o) : -1;
}
...
static int32_t sys_gui_list(uint32_t p, uint32_t max) {
  ...
  if (max > GUI_MAX_WINDOWS)
    max = GUI_MAX_WINDOWS;
  if (max > (uint32_t)(SIZE_MAX / sizeof(struct gui_window_desc)))
    return -1;
...
static int32_t sys_sysinfo(uint32_t p) {
  ...
  out->total_memory_kb =
      (uint32_t)(((uint64_t)pmm_total_frames() * PAGE_SIZE) / KB);
  out->used_memory_kb =
      (uint32_t)(((uint64_t)pmm_used_frames() * PAGE_SIZE) / KB);
...
static void collect_process(const struct process *proc, void *ctx) {
  struct proclist_state *s = (struct proclist_state *)ctx;
  if (s->count >= s->max)
    return;
  s->out[s->count].pid = (uint32_t)proc->pid;
  s->out[s->count].parent = (uint32_t)proc->parent;
  s->out[s->count].state = (uint32_t)proc->state;
  s->out[s->count].memory_kb = (proc->user_pages * PAGE_SIZE) / KB;
  s->out[s->count].cpu_ms = 0;
  strlcpy(s->out[s->count].name, proc->name, SYS_NAME_MAX);
  s->count++;
}
static int32_t sys_proclist(uint32_t p, uint32_t max) {
  struct proclist_state s;
  size_t bytes;
  s.out = (struct sys_proc *)(uintptr_t)p;
  s.max = max;
  s.count = 0;
  if (max > (uint32_t)(SIZE_MAX / sizeof(struct sys_proc)))
    return -1;
  bytes = (size_t)max * sizeof(struct sys_proc);
  if (!proc_user_write_ok(s.out, bytes))
    return -1;
  proc_foreach(collect_process, &s);
  return (int32_t)s.count;
}
static int32_t sys_time(uint32_t p) {
  struct sys_time *out = USER_STRUCT(p, struct sys_time);
  struct rtc_time now;
  if (!out)
    return -1;
  rtc_read(&now);
...
static int32_t sys_log(uint32_t p, uint32_t len) {
  char *out = (char *)(uintptr_t)p;
  if (!proc_user_write_ok(out, len))
    return -1;
...
static void syscall_handler(struct registers *regs) {
  uint32_t number = regs->eax, arg1 = regs->ebx, arg2 = regs->ecx,
           arg3 = regs->edx;
  int32_t result = -1;
  switch (number) {
  case SYS_EXIT:
  ...
  case SYS_WAIT: {
    int code = 0;
    if (arg2) {
      int f = proc_try_wait((int)arg1, &code);
      result = (f == 1) ? code : (f == 0 ? -1000 : -1);
    } else
  ...
  case SYS_GUI_INVALIDATE:
    result = wm_invalidate(proc_current(), arg1, (int)(int16_t)(arg2 & 0xFFFF),
                           (int)(int16_t)(arg2 >> 16), (int)(arg3 & 0xFFFF),
                           (int)(arg3 >> 16));
    break;
  case SYS_GUI_POLL:
  ...
  case SYS_GUI_TITLE: {
    char title[GUI_MAX_TITLE];
    if (proc_copy_user_string((const char *)(uintptr_t)arg2, title,
                              sizeof(title)) < 0)
      result = -1;
    else
