...
/* Theme */
#define COLOR_DESKTOP_TOP    RGB(0x10, 0x18, 0x28)
#define COLOR_DESKTOP_BOTTOM RGB(0x24, 0x33, 0x52)
#define COLOR_TITLE_ACTIVE   RGB(0x2C, 0x5C, 0xA8)
#define COLOR_TITLE_INACTIVE RGB(0x3A, 0x42, 0x50)
#define COLOR_TITLE_TEXT     RGB(0xF0, 0xF4, 0xF8)
#define COLOR_BORDER         RGB(0x14, 0x1A, 0x24)
#define COLOR_SHADOW         RGBA(0, 0, 0, 90)
...
static int resize_id, resize_start_w, resize_start_h, resize_start_x, resize_start_y;
...
static void draw_decorations(struct gfx_surface *screen, struct window *w, int active)
{
    ...
    /* Soft shadow on the two sides light would not reach. */
    gfx_blend_rect(screen, frame.x + 4, frame.y + frame.h, frame.w, 4, COLOR_SHADOW);
    gfx_blend_rect(screen, frame.x + frame.w, frame.y + 4, 4, frame.h, COLOR_SHADOW);

    gfx_fill_rect(screen, frame.x, frame.y, frame.w, WM_TITLE_HEIGHT, title_color);
    gfx_hline(screen, frame.x, frame.y, frame.w,
              active ? RGB(0x4E, 0x8A, 0xD8) : RGB(0x50, 0x58, 0x66));
    gfx_frame_rect(screen, frame.x, frame.y, frame.w, frame.h, COLOR_BORDER);

    gfx_text_clipped(screen, frame.x + 8, frame.y + 5, w->title,
                     active ? COLOR_TITLE_TEXT : RGB(0xB0, 0xB8, 0xC4), 1,
                     frame.w - 80);

    /* Minimise and close buttons. */
    button_y = frame.y + 6;
    gfx_fill_rect(screen, frame.x + frame.w - 44, button_y, 14, 14,
                  RGB(0xC8, 0xA8, 0x40));
    gfx_hline(screen, frame.x + frame.w - 41, button_y + 10, 8, RGB(0x20, 0x20, 0x20));

    gfx_fill_rect(screen, frame.x + frame.w - 24, button_y, 14, 14,
                  RGB(0xC8, 0x50, 0x48));
    for (int i = 0; i < 8; i++) {
        gfx_pixel(screen, frame.x + frame.w - 21 + i, button_y + 3 + i, RGB(0x20, 0x20, 0x20));
        gfx_pixel(screen, frame.x + frame.w - 21 + i, button_y + 10 - i, RGB(0x20, 0x20, 0x20));
    }
}
...
static void compose_rect(int rx, int ry, int rw, int rh)
{
    ...
    if (rx < 0) { rw += rx; rx = 0; }
    if (ry < 0) { rh += ry; ry = 0; }
    if (rx + rw > gfx_width())  rw = gfx_width() - rx;
    ...
    gfx_gradient_v(screen, rx, ry, rw, rh, COLOR_DESKTOP_TOP, COLOR_DESKTOP_BOTTOM);
    ...
    for (int i = 0; i < z_count; i++) {
        ...
        frame_rect(w, &frame);
        if (frame.x >= rx + rw || frame.y >= ry + rh ||
            frame.x + frame.w <= rx || frame.y + frame.h <= ry)
            continue;
...
static void handle_mouse_down(int x, int y, int button)
{
    ...
    if (hit_title && button == MOUSE_LEFT) {
        ...
        if (x >= frame.x + frame.w - 24 && x < frame.x + frame.w - 8) {
            send_simple(w, GUI_EVENT_CLOSE);
            return;
        }
        if (x >= frame.x + frame.w - 44 && x < frame.x + frame.w - 28) {
            wm_minimize(w->id);
            return;
        }
    ...
    if (!(w->flags & (GUI_FIXED | GUI_NODECOR)) &&
        x >= w->x + w->width - 12 && y >= w->y + w->height - 12) {
        resize_id = (int)w->id;
        resize_start_x = x;
        resize_start_y = y;
...
int wm_invalidate(struct process *owner, uint32_t id, int x, int y, int w_, int h)
{
    struct window *w = find(id);
...
int wm_resize_window(struct process *owner, uint32_t id, int width, int height)
{
    ...
    if (!w || w->owner != owner)
        return -1;
    if (width <= 0 || height <= 0 ||
        width > gfx_width() || height > gfx_height())
        return -1;
    if (width == w->width && height == w->height)
        return 0;
