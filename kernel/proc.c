...
virt_addr_t proc_sbrk(int32_t increment)
{
    ...
    if (increment > 0) {
        ...
        if (target >= limit)
            return 0;
        old_pages = (uint32_t)((ALIGN_UP(previous, PAGE_SIZE) - ALIGN_UP(proc->brk_start, PAGE_SIZE)) / PAGE_SIZE);
        new_pages = (uint32_t)((ALIGN_UP(target, PAGE_SIZE) - ALIGN_UP(proc->brk_start, PAGE_SIZE)) / PAGE_SIZE);
