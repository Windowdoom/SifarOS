...
static int send_arp(uint16_t opcode, const uint8_t dst_mac[6],
...
static int resolve_mac(uint32_t destination, uint8_t mac[6]) {
  ...
  uint32_t next_hop =
      ((destination & netmask) == (local_ip & netmask)) ? destination : gateway;
...
static int ipv4_payload(const uint8_t *frame, size_t length, uint8_t protocol,
                        uint32_t source, const uint8_t **payload,
                        size_t *payload_len) {
  const uint8_t *ip;
  uint32_t ihl;
  uint16_t total;
...
static int wait_udp(uint32_t source, uint16_t src_port, uint16_t dst_port,
  ...
  while (timer_ms() < deadline) {
    ...
    if (length < 0)
      return -1;
    if (length > 0 && ipv4_payload(frame, (size_t)length, IP_PROTO_UDP, source,
                                   &udp, &udp_len)) {
      uint16_t declared;
      size_t bytes;
...
static uint16_t tcp_checksum(uint32_t destination, const uint8_t *tcp,
                             size_t length) {
  uint8_t pseudo[12];
  uint32_t sum;
...
static int wait_tcp(uint32_t source, uint16_t src_port, uint16_t dst_port,
  ...
  while (timer_ms() < deadline) {
    ...
    if (length < 0)
      return -1;
    if (length > 0 && ipv4_payload(frame, (size_t)length, IP_PROTO_TCP, source,
                                   &tcp, &tcp_len)) {
      uint32_t header_len;
      size_t bytes;
  ...
  for (int attempt = 0; attempt < 3 && !connected; attempt++) {
    ...
    for (;;) {
      ...
      if ((packet.flags & (TCP_SYN | TCP_ACK)) == (TCP_SYN | TCP_ACK) &&
        ...
        local_seq++;
        if (send_tcp(remote, local_port, port, local_seq, remote_seq, TCP_ACK,
                     NULL, 0) < 0)
          return -3;
        connected = 1;
        break;
...

  {
    ...
    for (;;) {
      ...
      if (packet.payload_length) {
        ...
        (void)send_tcp(remote, local_port, port, local_seq, remote_seq, TCP_ACK,
                       NULL, 0);
      }
...
int net_ready(void) { return initialized && rtl8139_ready(); }
...
int net_http_get(const char *host, uint16_t port, const char *path, void *out,
                 size_t capacity) {
  uint32_t remote;
  uint32_t flags;
  int result;
  ...
  if (result == 0)
    result =
        http_transaction(remote, port, host, path, (uint8_t *)out, capacity);
